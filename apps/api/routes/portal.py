...
def public_create_ticket(payload: dict[str, Any], db: Session = Depends(get_db)) -> Any:
    ...
    data = {
        ...
        "priority": "Low",  # portal defaults low; agent raises
        "status": "Open",
    }
    # Use internal create (no actor or system)
    ticket = svc.create_ticket(data, actor={"username": "portal", "role": "viewer"})
    return {"ticket_id": ticket.get("ticket_id"), "status": "received", "note": "Track with your ticket id."}
