...
@router.get("/rules")
def list_rules(db: Session = Depends(get_db), _user: dict = Depends(get_current_user)) -> Any:
...
def trigger_rules(trigger: str, ticket: dict[str, Any], db: Session = Depends(get_db), _user: dict = Depends(get_current_user)) -> Any:
