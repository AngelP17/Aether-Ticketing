...
@router.get("/policies")
def list_policies(db: Session = Depends(get_db), _user: dict = Depends(get_current_user)) -> Any:
...
def sla_dashboard(db: Session = Depends(get_db), _user: dict = Depends(get_current_user)) -> Any:
    return SlaService(db).get_dashboard()

@router.get("/ticket/{ticket_id}")
def sla_for_ticket(ticket_id: str, db: Session = Depends(get_db), _user: dict = Depends(get_current_user)) -> Any:
